...
class inria(datasets.imdb):
	...
	raw_data = sio.loadmat(filename)['allBoxes'].ravel()
	...
	with open(filename) as f:
            data = f.read()
	import re
	objs = re.findall('person -?\d+ -?\d+ -?\d+ -?\d+', data)

        num_objs = len(objs)
        #print '{}, {}'.format(index, num_objs) 
    ...
    def _write_inria_results_file(self, all_boxes, output_dir):
        ...
        path = os.path.join(output_dir, 'results', self.name)
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            filename = os.path.join(path, comp_id+'_'+'det_' + self._image_set + '_' + cls + '.txt')
            with open(filename, 'wt') as f:
                for im_ind, index in enumerate(self.image_index):
                    dets = all_boxes[cls_ind][im_ind]
                    if dets == []:
                        #print "no detection bbs for cls {}".format(cls)
                        continue
                    # the VOCdevkit expects 1-based indices
                    for k in xrange(dets.shape[0]):
                        f.write('{} {:.1f} {:.1f} {:.1f} {:.1f} {:.4f}\n'.
                                format(im_ind + 1, 
